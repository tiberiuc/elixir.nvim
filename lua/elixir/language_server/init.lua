...
function M.debugger_path(opts)
  local new_opts = make_opts(opts)
  local params = { path = tostring(install_dir),
    repo = new_opts.repo,
    ref = new_opts.ref,
  ...
  local install_path =
  Path:new(params.path, params.repo, Utils.safe_path(params.ref), params.versions, "debugger.sh")

  return install_path.filename
